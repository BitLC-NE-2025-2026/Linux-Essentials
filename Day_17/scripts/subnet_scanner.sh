...
SUBNET_CHOICE=$(whiptail --title "Subnetz-Scanner & Discovery" \
                         --menu "Wählen Sie das zu scannende LAN-Segment aus:" $W_HEIGHT $W_WIDTH $W_LIST \
...
whiptail --title "Scan läuft" --infobox "Scanne $BASE_NET.$START_IP bis $BASE_NET.$END_IP parallel im Hintergrund...\nBitte warten (Dauer: ca. 2 Sek.)..." 8 75
...
for i in $(seq "$START_IP" "$END_IP"); do
    ...
    (
        ...
        if ping -c 1 -W 1 "$target_ip" >/dev/null 2>&1; then
            # Hostname rückauflösen falls DNS funktioniert
            host_name=$(getent hosts "$target_ip" | awk '{print $2}' || echo "")
            if [[ -n "$host_name" ]]; then
                echo "  ➜ $target_ip: [ONLINE] ($host_name)" >> "$TEMP_SCAN_LOG"
            else
                echo "  ➜ $target_ip: [ONLINE]" >> "$TEMP_SCAN_LOG"
            fi
        else
            echo "  ➜ $target_ip: [offline]" >> "$TEMP_SCAN_LOG"
        fi
    ) &
done

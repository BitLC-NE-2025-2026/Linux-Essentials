...
# Zweck: Dynamischer DNS-Benchmark mit 3-Packet-Ping & Whiptail TUI (FHD Optimiert)
# ==============================================================================
...
CHOICE=$(whiptail --title "DNS-Server Auswählen (Benchmark-Ergebnisse)" \
                  --menu "Wählen Sie den gewünschten DNS-Server. Die Latenz (Ping) wurde live ermittelt:" $W_HEIGHT $W_WIDTH $W_LIST \
...
if [[ -n "$CHOICE" ]]; then
    ...
    whiptail --title "DNS Aktualisiert" --msgbox "Der primäre DNS-Server wurde auf $CHOICE gesetzt!" 8 65
fi
